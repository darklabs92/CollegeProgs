...
for file_name in folderList:
    ...
    if ext == ".xml":
        aspects = asp.get_aspect(file_name)
        asp_cat = aspc.get_aspect_categories(aspects)
        asp_sent = asps.get_aspect_sentiment(aspects, file_name, categories = False)
        asp_sent_list.append([file_name[:-8], aspects, asp_sent])
...
temp_df.to_csv("for_cloud.csv", encoding = "UTF-8")
